...
export default function SwipeRedirectPage() {
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (user) => {
            ...
            try {
                ...
                if (account.account_type === 'seeker') {
                    router.push('/pages/swipe/seeker');
                    return;
                }

                if (account.account_type === 'recruiter') {
                    router.push('/pages/swipe/recruiter');
                    return;
                }

                router.push('/pages/login');
            } catch (error) {
                console.error('Swipe redirect error:', error);
                router.push('/pages/login');
            }
        });
    ...
    return (
        <div className="flex min-h-screen items-center justify-center bg-[#FFF8DC] text-black">
            <p className="text-lg font-semibold">Opening swipe view...</p>
        </div>
    );
}